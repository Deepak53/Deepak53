...
if (!keys.length) {
  console.error(
    chalk.red(
      `No environment variables found that start with ${prefixes.join(' ')}`
    )
  )
  process.exit(1)
}
...
prefixes.forEach(p =>
  (keys.some(k => k.startsWith(p)) ? found : notFound).push(p)
)

console.info(
  chalk.green.bold(
    `Found environment variables that start with ${found.join(' ')}:`
  )
)
keys.forEach(key => {
...
if (notFound.length) {
  console.info(
    chalk.yellow(
      `No environment variables found that start with ${notFound.join(' ')}`
    )
  )
}
